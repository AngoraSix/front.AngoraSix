...
const ProjectsComponent = ({ managements }) => {
  ...
  const adminProjects = managements.collection.filter((mgmt) => {
    const member = mgmt.members?.find(
      (m) => m.contributorId === session?.user?.id
    )
  ...
  const handleProjectClick = (managementId) => {
    router.push(`${resolveRoute(ROUTES.management.dashboard, managementId)}`)
  }

  const handleGettingStarted = () => {
    router.push(ROUTES.platform.gettingStarted)
  }
  ...
  const renderProjectCard = (mgmt, isAdmin) => {
    const activeMembers =
      mgmt.members?.filter((m) => m.status === 'active') || []

    return (
      <Grid item xs={12} sm={6} md={4} key={mgmt.id}>
        <Link href={resolveRoute(ROUTES.management.dashboard, mgmt.id)}>
          <Card
          ...
          >
            <CardContent>
              ...
              <Box display="flex" alignItems="center" gap={1}>
                <Chip
                  label={t(`status.${mgmt.status?.toUpperCase()}`)}
                  size="small"
                  variant="outlined"
                />
              </Box>
        ...
        <Container maxWidth="lg" className="projects-container">
          <Box py={6}>
            ...
            >
              ...
              <Link href={ROUTES.platform.gettingStarted}>
                <Button
                  variant="contained"
                  color="primary"
                  startIcon={<MapIcon />}
                  size="large"
                >
                  {t('actions.gettingstarted')}
                </Button>
              </Link>
              <Link href={ROUTES.management.new}>
                <Button
                  variant="contained"
                  color="primary"
                  startIcon={<AddIcon />}
            ...
            {managements.collection.length === 0 ? (
              ...
              <>
                ...
                {adminProjects.length > 0 && (
                  <Box mb={6}>
                    ...
                    <Grid container spacing={3}>
                      {adminProjects.map((mgmt) =>
                        renderProjectCard(mgmt, true)
                      )}
                    </Grid>
                  </Box>
                )}
                ...
                {adminProjects.length > 0 && memberProjects.length > 0 && (
                  <Divider sx={{ my: 4 }} />
                )}
                ...
                {memberProjects.length > 0 && (
                  <Box mb={4}>
                    ...
                    <Grid container spacing={3}>
                      {memberProjects.map((mgmt) =>
                        renderProjectCard(mgmt, false)
                      )}
                    </Grid>
                  </Box>
                )}

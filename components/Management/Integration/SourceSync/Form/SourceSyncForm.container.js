...
import {
  ...
  INTERCOMMUNICATION_KEYS,
} from '../../ManagementIntegration.properties'
...
const SourceSyncFormContainer = ({ sourceSync }) => {
  ...
  const formData = requiredDataForStep.map((data) => ({
    [data.key]: data.value,
  }))
  ...
  const onFormChange = (stepIndex, property, isMultiple) => (eventOrValue) => {
    const value = eventOrValue.target ? eventOrValue.target.value : eventOrValue
    if (isMultiple) {
      dispatch(toggleSelectionAction({ stepIndex, property, value }))
    } else {
  ...
  const onContinueSync = async () => {
    ...
    if (formState.requiredDataForStep.every((item) => _hasBeenFilled(item))) {
      const sourceSyncResponse = await api.front.submitSourceSyncStep(
        sourceSync.id,
        formState.currentStepObj.index,
        formState.formData[formState.currentStepObj.index]
      )
      ...
      if (sourceSyncStatus === SOURCE_SYNC_STATUS.REGISTERED) {
        const isMultipleStepProcess =
          INTEGRATION_QUERY_PARAMS.multipleStepProcess in router.query
        if (isMultipleStepProcess) {
          const matchPlatformUsersPage = `${resolveRoute(
            ...
            isMultipleStepProcess
              ? `?${INTEGRATION_QUERY_PARAMS.multipleStepProcess}`
              : ''
          }`
          setTimeout(() => {
          ...
          window.opener.postMessage(
            {
              type: INTERCOMMUNICATION_KEYS.sourceSyncConfigCompleted,
              data: sourceSyncResponse,
            },
  ...
  const _hasBeenFilled = (item) => {
    ...
    return (
      Object.hasOwn(stepFormData, item.key) &&
      stepFormData[item.key] != null &&
      stepFormData[item.key] != ''
    )
  }

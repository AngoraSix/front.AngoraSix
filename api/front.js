...
class FrontAPI {
  ...
  async saveNewProject(projectBody) {
  ...
  async getSourceSync(sourceSyncId) {
    const { data } = await this.axios.get(`api/integrations/${sourceSyncId}`)
    return data
  }
  ...
  async requestFullSync(sourceSyncId) {
    const { data } = await this.axios.patch(
      `api/integrations/${sourceSyncId}`,
      createPatchBody(PATCH_SUPPORTED_OPERATIONS.ADD, `events`, {
  ...
  async startPlatformUsersMatch(sourceSyncId, projectContributors) {
    const { data } = await this.axios.post(
      `api/integrations/${sourceSyncId}/mappings/users`,
      {
        projectContributors,
      }
    )
  ...
  async saveSurveyResponse(surveyResponseBody, surveyKey) {
    const { data } = await this.axios.post(
      `api/surveys/${surveyKey}/responses`,
      surveyResponseBody
    )
    return data
  }
  ...
  async suscribe(
    email,
    { newsletterList = false, betaList = false, source = 'general' } = {}
  ) {
  ...
  // @TODO remove when implementing Trello-FNxJ9zGB
  wakeupMailingSvc() {
    this.axios.post(`api/mailing/wakeup`)
  }
}
